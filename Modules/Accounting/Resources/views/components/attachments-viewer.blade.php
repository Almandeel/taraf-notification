...
@if ($view == 'timeline')
    <div class="timeline">
        @if (count($attachments))
            @foreach ($attachments->groupBy(function($val) {
            return \Carbon\Carbon::parse($val->created_at)->format('Y-m-d'); }) as $key => $day)
                <div class="time-label">
                    <span class="bg-primary">{{ $key }}</span>
                </div>
                @foreach ($day as $attachment)
                    @if ($attachment->isNote() && ($component == 'all' || $component == 'notes'))
                        <div>
                            ...
                            <div class="timeline-item">
                                ...
                                <h3 class="timeline-header"><span>{{ __('accounting::global.offices.' . $attachment->office) }}: </span><strong>{{ $attachment->auth() ? $attachment->auth()->name : '' }}</strong></h3>
                                    ...
                                    <div class="timeline-footer">
                                        ...
                                        @permission('attachments-update')
                                            <button class="btn btn-warning btn-xs" data-modal="attachment" 
                                                ...
                                                data-action="{{ route('attachments.update', $attachment) }}">
                                        ...
                                        @permission('attachments-delete')
                                            ...
                                            <form id="deleteFrom-{{$attachment->id}}" action="{{ route('attachments.destroy', $attachment) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        @endpermission
                        ...
                        <div>
                            ...
                            <div class="timeline-item">
                                ...
                                <h3 class="timeline-header">
                                    <span>{{ __('accounting::global.offices.' . $attachment->office) }}: </span><strong>{{ $attachment->auth() ? $attachment->auth()->name : '' }}</strong>
                                </h3>
                                <div class="timeline-body">
                                    {{ $attachment->name }}
                                ...
                                <div class="timeline-footer">
                                    <a href="{{ $attachment->isImage() ? $attachment->getFileURL() : route('attachments.show', $attachment) }}"
    ...
    <div class="btn-group text-center">
        ...
        <button type="button" class="btn btn-default btn-sm btn-add" data-modal="attachment"
            data-action="{{ route('attachments.store') }}" data-is-note="false" 
        ...
        <button type="button" class="btn btn-success btn-sm btn-add" data-modal="attachment"
            data-action="{{ route('attachments.store') }}" data-is-note="true" 
            data-attachable-id="{{ $attachableId }}" data-attachable-type="{{ $attachableType }}">
        ...
        <tbody>
            ...
            @foreach ($attachments as $attachment)
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ $attachment->isImage() ? $attachment->getFileURL() : route('attachments.show', $attachment) }}" @if($attachment->isImage()) data-toggle="lightbox" data-type="image" @endif class="btn btn-info btn-xs">
                            <i class="fas fa-eye"></i>
                            <span class="d-xs-none d-md-inline">@lang('accounting::global.show')</span>
                        </a>
                        @if ($attachment->isEdittable())
                    ...
                    <th colspan="4">
                        <div class="btn-group text-center">
                            ...
                            <button type="button" class="btn btn-default btn-sm btn-add" 
                                data-modal="attachment" data-action="{{ route('attachments.store') }}" data-is-note="false"
                                data-attachable-id="{{ $attachableId }}" data-attachable-type="{{ $attachableType }}"
                            >
                                <i class="fa fa-file"></i>
                                <span class="d-none d-md-inline">@lang('accounting::global.add_file')</span>
                            ...
                            <button type="button" class="btn btn-success btn-sm btn-add" 
                                data-modal="attachment" data-action="{{ route('attachments.store') }}" data-is-note="true"
                                data-attachable-id="{{ $attachableId }}" data-attachable-type="{{ $attachableType }}"
                            >
                                <i class="fa fa-sticky-note"></i>
                                <span class="d-none d-md-inline">@lang('accounting::global.add_note')</span>
